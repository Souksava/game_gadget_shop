...
<html>
  ...
  <body >
    ...
    ?>
        <div class="container font14">
            <div>
                <form action="Report_revenue.php" method="POST" id="formReport">
            ...
            <div class="table-responsive">
                <table class="table" style="width: 1800px;">
                    <tr>
                        ...
                        <th style="width: 60px;" scope="col">ສະຖານະ</th>
                    ...
                    <?php
                        $sql = "select d.sell_id,pro_name,d.qty,d.price,d.qty*d.price as total,p.img_path,sell_date,s.status,status_cash,sell_type,color_name,unit_name,brand_name,cate_name,cated_name,emp_name,cus_name from selldetail d left join sell s on d.sell_id=s.sell_id left join product p on d.pro_id=p.pro_id left join categorydetail i on p.cated_id=i.cated_id left join category c on i.cate_id=c.cate_id left join brand b on p.brand_id=b.brand_id left join unit u on p.unit_id=u.unit_id left join product_color r on d.color_id=r.color_id left join employees y on s.emp_id=y.emp_id left join customers t on s.cus_id=t.cus_id;";
                    ...
                    <tr>
                        ...
                        <td>
                            ...
                            <?php echo $row['cated_name']; ?> <br><br>
                        </td>
                        <td> 
                            <?php echo $row['qty']; ?> <?php echo $row['unit_name']; ?><br>
                            <?php 
                                    if($row['color_name'] != ""){
                                        echo"ສີ".$row['color_name']."";
                                    }
                                    else{
                                        echo"";
                                    }
                    ...
                    <?php
                        ...
                        $sql2 = "select sum(d.qty*d.price) as amount from selldetail d left join sell s on d.sell_id=s.sell_id left join product p on d.pro_id=p.pro_id left join categorydetail i on p.cated_id=i.cated_id left join category c on i.cate_id=c.cate_id left join brand b on p.brand_id=b.brand_id left join unit u on p.unit_id=u.unit_id left join product_color r on d.color_id=r.color_id left join employees y on s.emp_id=y.emp_id left join customers t on s.cus_id=t.cus_id;";
                        $result2 = mysqli_query($link,$sql2);
                        $row2 = mysqli_fetch_array($result2, MYSQLI_ASSOC);
                        $sql3 = "select sum(cupon_price) as cupon_price from sell;";
                        $result3 = mysqli_query($link,$sql3);
                        $row3 = mysqli_fetch_array($result3, MYSQLI_ASSOC);
                        $sql4 = "select sum(delivery) as delivery from sell;";
                        $result4 = mysqli_query($link,$sql4);
                        $row4 = mysqli_fetch_array($result4, MYSQLI_ASSOC);
                        $amount = $row2['amount'] - $row3['cupon_price'];
                        $newamount = $amount + $row4['delivery'];
                    ?>
                    ...
                    <tr align="right" style="font-size: 26px;">
                        <td colspan="9">ລວມມູນຄ່າ: </td>
                        <td colspan="4"><?php echo number_format($amount,2); ?> ກີບ</td>
                    </tr>
                    <tr align="right" style="font-size: 26px;">
                        <td colspan="9">ຮັບຈາກການຂົນສົ່ງ: </td>
                        <td colspan="4"><?php echo number_format($row4['delivery'],2); ?> ກີບ</td>
                    </tr>
                    <tr align="right" style="font-size: 26px;">
            ...
            <div class="table-responsive">
                <table class="table" style="width: 1800px;">
                    ...
                    <?php
                        ...
                        $result3 = mysqli_query($link,$sql3);
                        $row3 = mysqli_fetch_array($result3, MYSQLI_ASSOC);
                        $sql4 = "select sum(delivery) as delivery from sell where sell_date between '$date1' and '$date2';";
                        $result4 = mysqli_query($link,$sql4);
                        $row4 = mysqli_fetch_array($result4, MYSQLI_ASSOC);
                        $amount = $row2['amount'] - $row3['cupon_price'];
                        $newamount = $amount + $row4['delivery'];
