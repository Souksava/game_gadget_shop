...
function ShowData(){
    ...
    $rate = $_POST['rate'];
    if(isset($_POST['btn'])){
        $sql = "select pro_name,unit_name,cated_name,cate_name,brand_name,d.qty,d.price,d.qty*d.price as total from orderdetail d left join orders o on d.order_id=o.order_id left join product p on d.pro_id=p.pro_id left join categorydetail l on p.cated_id=l.cated_id left join category c on l.cate_id=c.cate_id left join brand b on p.brand_id=b.brand_id left join unit u on p.unit_id=u.unit_id where d.order_id='$order_id';";
