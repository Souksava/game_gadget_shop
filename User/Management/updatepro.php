...
<html>
    ...
    <body >
        ...
        <?php 
            $id = $_GET['id'];
            $sqlget = "select pro_id,pro_name,p.qty,price,p.cated_id,cated_name,p.unit_id,unit_name,p.brand_id,brand_name,guarantee,type,promotion,qtyalert,p.img_path,p.status from product p left join categorydetail c on p.cated_id=c.cated_id left join unit u on p.unit_id=u.unit_id left join brand b on p.brand_id=b.brand_id where pro_id='$id'";
            ...
            <form action="updatepro.php" id="update" method="POST" enctype="multipart/form-data">
                <div class="row" align="left">
                    ...
                    <div class="col-xs-12 col-sm-6 form-group">
                        ...
                        <input type="number" min="0" name="guarantee" class="form-control" value="<?php echo $row['guarantee']; ?>" placeholder="ເວລາຮັບປະກັນ">
                    </div>
                    <div class="col-xs-12 col-sm-6 form-group">
            ...
            if(isset($_POST['btnUpdate'])){
                ...
                elseif(trim($status) == ""){
                    ...
                    if($_FILES['img_path']['name'] == ""){
                        $sqlupdate = "update product set pro_name='$pro_name',qty='$qty',price='$price',cated_id='$cate_id',unit_id='$unit_id',brand_id='$brand_id',guarantee='$guarantee',type='$type',promotion='$promotion',qtyalert='$qtyalert',status='$status' where pro_id='$pro_id';";
                        ...
                        //ສິນສຸດການຕັ້ງຊື່
                        $sqlupdate = "update product set pro_name='$pro_name',qty='$qty',price='$price',cated_id='$cate_id',unit_id='$unit_id',brand_id='$brand_id',guarantee='$guarantee',type='$type',promotion='$promotion',qtyalert='$qtyalert',img_path='$pro_image',status='$status' where pro_id='$pro_id';";
                        $resultupdate = mysqli_query($link,$sqlupdate);
                        if(!$resultupdate){
                            echo"<script>";
                            echo"alert('ບໍ່ສາມາດແກ້ໄຂຂໍ້ມູນໄດ້');";
